...
#' @param scale_e Niche height for colonisation, see 'details'.
...
#' @details The metacommunity describes the possible biological space for a `flume` model. It consists
#' of a few named elements:
...
#' Must be a matrix with one row per species (`nsp`), one column per niche axis (`nrx`). For a single
#' niche axis, a vector with one entry per species is also accepted. The location is the mean of
#' the Gaussian colonisation function.
#'
...
#'   * A **vector** of length `nsp`: each species has a different breadth, but breadth is the same for
#'   each niche axis.
#'   * A **vector** of length `nrx`: niche breadth varies by axis, but all species have the same
#'   breadth for each niche axis. If `nrx == nsp`, then this option is not possible, instead the former
#'   applies.
#'   * A **matrix** of `nsp` rows and `nrx` columns: each species-resource combo has a unique breadth,
#'   but all resources are orthogonal. In other words, performance for a given resource cannot
#'   depend on the concentration of any other resource.
...
#'
#' Dispersal parameters are `alpha` and `beta`, for specifying active (i.e., either up- or downstream)
#' or passive (downstream only) dispersal, respectively. These can either be a scalar, in which case
#' all species have the same dispersal ability, or a vector of length `nsp`.
#'
...
metacommunity = function(location, breadth = 1, scale_c = 0.5, scale_e = 0.2, alpha = 0.05,
	...
	comm[['species']] = mapply(species, location = location, breadth = breadth, scale_c = scale_c,
		   scale_e = scale_e, alpha = alpha, beta = beta, SIMPLIFY = FALSE, USE.NAMES = FALSE)
...
#' @rdname check_par
#' @keywords internal
.check_par_scale = function(p, nsp, default = 1) {
	if(!('scale' %in% names(p)))
		p[['scale']] = default
	if(length(p[['scale']]) == 1)
		p[['scale']] = rep(p[['scale']], nsp)
	if(length(p[['scale']]) != nsp)
		stop("Scale parameter must be missing, a single value, or 1 value per species")
	p
}
...
.check_r_scale = function(p, nsp, nrx) {
	if(length(p) == 1)
		p = rep(p, nsp)
	if(length(p) == length(nsp)) {
		p = matrix(p, nrow=nsp, ncol=nrx)
	} else if(length(p) == length(nrx)) {
		p = matrix(p, nrow=nsp, ncol=nrx, byrow=TRUE)
	}
...
#' `location` must be a location vector of length `nx`, and `width` must be a variance covariance matrix
#' with `dim=c(nx,nx)`.
#'
