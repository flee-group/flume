...
The height of the niche (in the rightmost panel) also determines the intensity of resource use; a species that is performing highly in a particular environment will also consume more resources. This value is scaled by how "greedy" species are for resources using the `r_scale` variable in the metacommunity.This should be a matrix, with one row per species and one column per resource. By default this is set to a low value and is the same for all species. Here we modify it to make species 2 relatively greedier than species 1.
