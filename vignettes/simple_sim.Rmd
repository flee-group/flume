...
By default, species are dispersed in niche space, with a fundamental niche following a gaussian shape. The more species, the more tightly packed the niche space:

```{r fig.height = 3, fig.width = 7}
mcom = metacommunity(n_species = 3, nx = 1)
plot(mcom)

mcom = metacommunity(n_species = 5, nx = 1)
plot(mcom, ylab = "Performance", yaxt='n', bty='l', lwd=6, xaxt='n', cex.lab=2, mgp=c(2,0.5,0))
```

For this exercise, we will use a 2-species system. In addition to seeing all species' fundamental niches, we can see individual colonisation and extinction functions using the `mcom$species` attribute.

The height of the niche also determines species' resource uses. The model allows some species to be "greedier," i.e., to use more resources at a given niche height; this is done by setting the `r_scale` parameter. Here we read it's default value, then triple the greediness of species 2.

```{r fig.height = 3, fig.width = 7}
mcom = metacommunity(n_species = 2, nx = 1)
r_scale(mcom)
r_scale(mcom) = r_scale(mcom) * c(1,3)
...
The adjacency matrix describes the spatial relationships among nodes. The matrix must be square, with one row and column per node. For two sites `i` and `j`, `adj[i,j]` will be nonzero if `i` is upstream of `j`.
...
plot(network, layout = layout, edge.arrow.size = 0.2, edge.width = 20)
```
...
```{r fig.height = 5, fig.width = 5}
quartz(file="~/Desktop/network.png", w=8, h = 8, dpi=150, type='png')
plot(network, layout = layout, edge.arrow.size = 1, edge.width = 20)
legend("bottomleft", legend=c("1", rep("", 8), "0"), bty='n', title="", 
	   fill=rev(scales::col_numeric("PuBu", range(state(network)))(state(network))))
dev.off()
```
...
plot(sim$networks[[1]], t = 1, zlim=c(0,1), layout=layout, edge.arrow.size = 0.2)
plot(sim$networks[[1]], t = 11, zlim=c(0,1), layout=layout, edge.arrow.size = 0.2)

plot(sim$networks[[1]], t = 1, variable = 'site_by_species', layout = layout, edge.arrow.size = 0.2)
plot(sim$networks[[1]], t = 11, variable = 'site_by_species', layout = layout, edge.arrow.size = 0.2)

```

`flume` simulations are stochastic, so it's generally helpful to run a large number of simulations and look at average results over time. Here we create a model and run 64 replicates for 1000 time steps.

```{r}
sim = flume(sp_pool, network, dt = 1)
sim = run_simulation(sim, nt = 1000, reps=64)
plot(sim, variable = "occupancy", type="network")
plot(sim, variable = "occupancy", type="reach")
