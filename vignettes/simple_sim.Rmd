...
The metacommunity is an S3 object created by `flume` that describes all of the species that can
...
mc = metacommunity(scenario  = uniform_niches(nsp = 3, nr = 1))
plot(mc)
```

By default, all of the resources in the model are assumed to be `dynamic`; these represent
chemicals, nutrients, and other resources that are consumed by species and will run out if not
replenished. Resources may instead be `static`; these represent fixed habitat features (e.g. light,
habitat structure) that do not change. Here, we generate 
