...
The parameter list controls how the model operates. The list should be named, and each element of the list should be a scalar (for parameters that will apply to all sites/species) or a vector (e.g., if species vary in dispersal ability, then the dispersal params can be a vector with one entry per species).

A good starting point is the function `flume_params()`, which will return a parameter list with all required elements, set to sensible defaults. Individual parameters can then be edited as desired. It is recommended to use the function `validate_flume_params()` after editing, to make sure the parameter list still contains all required elements and that all parameters are within allowable ranges.
...
params = list(
	alpha = 0, 
	beta = c(0.5,1),
	gamma = matrix(c(0,1,1,0), ncol=length(sp_pool))
)
```
